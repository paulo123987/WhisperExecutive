...
O **Whisper Executive Transcriptor** é uma plataforma completa para transcrição e análise de áudios de call center. Combina o poder do **OpenAI Whisper** para transcrição em Português do Brasil com:
...
```text
.
├── app.py                              # Aplicação principal Streamlit (5 abas)
├── regex_callcenter_br.json            # Padrões regex regionais (BR call center)
├── requirements.txt                    # Dependências do projeto
├── src/
│   ├── __init__.py
│   ├── transcriber.py      # Motor de transcrição + diarização (Whisper)
...
openai-whisper   — Motor de transcrição STT
